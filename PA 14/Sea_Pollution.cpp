...
void Sea_Pollution::set_surface_area(float surf_area) {
	if (surf_area > 0) {
		...
		std::cout << "\nFloat value for surface_area was less than or equal to zero which is invalid, triggeing EXIT_FAILURE\n";
		exit(EXIT_FAILURE);
	}
}
...
void Sea_Pollution::set_pollution_area(float poll_area) {
	if (poll_area > 0) {
		...
		std::cout << "\nFloat value for pollution_area was less than or equal to zero which is invalid, triggeing EXIT_FAILURE\n\n";
		exit(EXIT_FAILURE);
	}
}
...
void Sea_Pollution::set_salinity_level(float sal_level) {
	if (sal_level > 0) {
		...
		std::cout << "\nFloat value for salinity_level was less than or equal to zero which is invalid, triggeing EXIT_FAILURE\n\n";
		exit(EXIT_FAILURE);
	}
}
